...
describe('1) createUser', () => {
    it('Пользователь с обязательными полями', () => {
        const user = createUser(1, 'Дмитрий');
    ...
    it('Пользователь со всеми полями', () => {
        const user = createUser(2, 'Жека', 'jeka@example.com');
        
        expect(user).toEqual({
            id: 2,
            name: 'Жека',
            email: 'jeka@example.com',
            isActive: true
        });
    });

    it('isActive = false', () => {
        const user = createUser(3, 'Степан', undefined, false);
...
describe('4) getStatusColor', () => {
    ...
    it('green для inactive', () => {
        expect(getStatusColor('inactive')).toBe('green');
    });

    it('black для new', () => {
        expect(getStatusColor('new')).toBe('black');
    });
});
